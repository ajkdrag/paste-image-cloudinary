...
(function(CompressionLevel2) {
  CompressionLevel2["None"] = "none";
  CompressionLevel2["Low"] = "low";
  CompressionLevel2["Medium"] = "medium";
  CompressionLevel2["High"] = "high";
...
var ImageProcessor = class {
  ...
  static processWithCanvas(img, originalFile, options) {
    return new Promise((resolve, reject) => {
      ...
      const targetWidth = options.downsample ? Math.round(img.naturalWidth * 0.25) : img.naturalWidth;
      const targetHeight = options.downsample ? Math.round(img.naturalHeight * 0.25) : img.naturalHeight;
      canvas.width = targetWidth;
      canvas.height = targetHeight;
      ctx.imageSmoothingEnabled = true;
      ctx.imageSmoothingQuality = "high";
      ctx.drawImage(img, 0, 0, targetWidth, targetHeight);
      const {outputFormat, quality} = this.getOutputSettings(originalFile.type, options.compressionLevel);
      canvas.toBlob((blob) => {
        if (!blob) {
  ...
  static getOutputSettings(inputFormat, compressionLevel) {
  ...
  static async getFileSize(file) {
    ...
    return sizeInKB < 1024 ? `${Math.round(sizeInKB)} KB` : `${(sizeInKB / 1024).toFixed(1)} MB`;
  }
};
ImageProcessor.COMPRESSION_QUALITY = {
  [CompressionLevel.None]: 1,
  [CompressionLevel.Low]: 0.8,
  [CompressionLevel.Medium]: 0.6,
  [CompressionLevel.High]: 0.4
};
ImageProcessor.LOSSY_FORMATS = [
  "image/jpeg",
  "image/jpg",
  "image/webp"
];
...
`;
  ...
  replacePlaceholder(placeholder, imageUrl) {
  ...
  replaceText(target, replacement) {
    ...
    for (let i = 0; i < this.editor.lineCount(); i++) {
      ...
      if (ch !== -1) {
        this.editor.replaceRange(replacement, {line: i, ch}, {line: i, ch: ch + target.length});
        break;
      }
    }
  }
...
// src/components/upload-modal.ts
var import_obsidian = __toModule(require("obsidian"));
var UploadModal = class extends import_obsidian.Modal {
  constructor(app, defaultFileName, originalFile, onSubmit) {
    super(app);
  ...
  createSettings(contentEl, previewSizeEl) {
    new import_obsidian.Setting(contentEl).setName("Image name").setDesc("Enter a name for your image").addText((text) => text.setValue(this.options.fileName).onChange((value) => {
      this.options.fileName = value;
    }));
    const isLossyFormat = this.originalFile.type.toLowerCase().match(/jpe?g|webp/);
    contentEl.createEl("p", {
      cls: "format-info",
      text: isLossyFormat ? `Image format: ${this.originalFile.type} (supports compression)` : `Image format: ${this.originalFile.type} (will be converted to WebP for compression)`
    });
    new import_obsidian.Setting(contentEl).setName("Downsample image").setDesc("Reduce image resolution by 50%").addToggle((toggle) => toggle.setValue(this.options.downsample).onChange((value) => {
      this.options.downsample = value;
      this.updatePreview(previewSizeEl);
    }));
    new import_obsidian.Setting(contentEl).setName("Compression level").setDesc("Choose image compression level").addDropdown((dropdown) => dropdown.addOptions({
    ...
    }).setValue(this.options.compressionLevel).onChange((value) => {
      this.options.compressionLevel = value;
      this.updatePreview(previewSizeEl);
    }));
    new import_obsidian.Setting(contentEl).addButton((btn) => btn.setButtonText("Upload").setCta().onClick(() => {
      this.close();
      this.onSubmit(this.options);
    }));
    this.updatePreview(previewSizeEl);
  }
  async updatePreview(previewSizeEl) {
    ...
    try {
      const processedFile = await ImageProcessor.processImage(this.originalFile, {
      ...
      if (this.options.downsample || this.options.compressionLevel !== CompressionLevel.None) {
        previewSizeEl.setText(`Estimated size after processing: ${this.previewSize} (${reduction}% smaller)`);
      } else {
      ...
      new import_obsidian.Notice("Failed to generate size preview");
    } finally {
      this.processingPreview = false;
    }
  }
...
var SettingsTab = class extends import_obsidian2.PluginSettingTab {
  ...
  createSetting(field) {
    new import_obsidian2.Setting(this.containerEl).setName(field.name).setDesc(field.description).addText((text) => {
      text.setPlaceholder(field.placeholder).setValue(this.plugin.settings[field.key]).onChange(async (value) => {
...
var CloudinaryUploader = class extends import_obsidian3.Plugin {
  constructor() {
    super(...arguments);
    this.handlePaste = async (event, editor) => {
  ...
  async handleImageUpload(file, editor) {
    try {
      ...
      const defaultName = activeFile ? activeFile.basename + "-" + Math.random().toString(36).substring(2, 8) : file.name.replace(/\.[^/.]+$/, "");
      new UploadModal(this.app, defaultName, file, async (options) => {
        try {
          const processedFile = await ImageProcessor.processImage(file, {
            downsample: options.downsample,
            compressionLevel: options.compressionLevel
          });
          const response = await this.cloudinaryService.uploadImage(processedFile, options.fileName);
